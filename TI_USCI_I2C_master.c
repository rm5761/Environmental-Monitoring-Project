...
//TI_USCI_I2C_VEML
//-----------------------------------------------------------------------------
void TI_USCI_I2C_VEML( unsigned char modein)
{
...
//TI_USCI_VEML_SETUP
//--------------------------------------------------------------------------
void TI_USCI_VEML_SETUP(unsigned char slave_address, unsigned char prescale, unsigned char *fieldt, unsigned char *fieldr){
    P1SEL |= SDA_PIN + SCL_PIN;                 // Assign I2C pins to USCI_B0
    UCB0CTL1 = UCSWRST;                        // Enable SW reset
    UCB0CTL0 = UCMST + UCMODE_3 + UCSYNC;       // I2C Master, synchronous mode
    UCB0CTL1 = UCSSEL_2 + UCSWRST;              // Use SMCLK, keep SW reset
    ...
    TI_transmit_field = fieldt;
    TI_receive_field = fieldr;
    byteCtr = 1;
    UCB0CTL1 |= UCTR + UCTXSTT;                 // I2C TX, start condition
